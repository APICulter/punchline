...
io.on("connection", function (socket) {
	...
	socket.on("createGame", function () {
		...
		game.pin = Math.floor(Math.random() * 100);
		// game.hostSocketId = ID; // careful, check to remove this usage

		//should create room now
		games.push(game);
		hosts.push(host);

		//on rejoint la room qui porte le pin de la game
		socket.join(game.pin);

		socket.emit("newGame", host, game);
		// } else {
		// socket.emit("game already exists", game.pin);
		// }
	});

	socket.on("maintainSocket", function (pin, hostID) {
	...
	socket.on("joinRoom", function (data) {
		...
		if (typeof game !== "undefined") {
			...
			if (typeof player === "undefined") {
				...
				// socket.join(game.pin);
				// io.to(game.host.id).emit("newJoiner", data.user);
